...
module.exports = {
  name: commandName,
  category: "moderation",
  description: "Ban a user from your guild and instantly unban it, this serves to remove the member's messages.",
  ...
  run: async function run(bitch, message, args) {
      ...
      return message.channel.createMessage(
        {
          embed: {
            ...
            description: "You need to tag guild member. `softban`",
            color: c
          }
        }
      )
    ...
    member.createMessage(`You've been banned from **${message.guild.name}**. *Reason:* \`${reason}\``);
